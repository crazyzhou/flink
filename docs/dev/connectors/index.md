...
The [predefined data sources]({% link dev/datastream_api.md %}#data-sources) include reading from files, directories, and sockets, and
ingesting data from collections and iterators.
The [predefined data sinks]({% link dev/datastream_api.md %}#data-sinks) support writing to files, to stdout and stderr, and to sockets.
...
Flink offers an API for [Asynchronous I/O]({% link dev/stream/operators/asyncio.md %})
...
The [Queryable State]({% link dev/stream/state/queryable_state.md %}) interface
